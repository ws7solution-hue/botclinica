                ...
                {activeTicket.messages.map((m, i) => (
                  <div key={i}>
                    <div className={`max-w-[80%] px-3 py-2 rounded-xl text-xs font-sans ${
                      m.role === 'agent'
                        ? 'bg-[#1A6FA8] text-white self-start rounded-tl-none ml-0'
                        : 'bg-slate-100 text-slate-800 self-end rounded-tr-none ml-auto'
                    }`} style={{ alignSelf: m.role === 'agent' ? 'flex-start' : 'flex-end', display: 'inline-block' }}>
                      {m.text}
                    </div>
                    <p className={`text-[10px] text-slate-400 font-sans mt-0.5 ${m.role === 'agent' ? 'text-left' : 'text-right'}`}>
